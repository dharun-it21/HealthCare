...
def create_report(request, pk):
    if request.user.is_labworker:
        ...
        if request.method == 'POST':
            ...
            try:
                send_mail(subject, plain_message, 'krishnakumar.it21@bitsathy.ac.in',  [patient_email], html_message=html_message, fail_silently=False)
...
def accept_doctor(request,pk):
    ...
    try:
        send_mail(subject, plain_message, 'krishnakumar.it21@bitsathy.ac.in',  [doctor_email], html_message=html_message, fail_silently=False)
    except BadHeaderError:
        return HttpResponse('Invalid header found')
